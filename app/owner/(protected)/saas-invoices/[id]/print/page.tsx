    ...
    <main className="mx-auto max-w-3xl bg-white p-12 text-sm text-slate-900 print:p-8">
      ...
      <div className="flex items-start justify-between border-b pb-6">
        ...
        <div className="text-right">
          ...
          <div className="mt-2 text-xs text-slate-600">Issued {new Date(invoice.issuedAt).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</div>
      ...
      <div className="mt-6 grid grid-cols-2 gap-6">
        <div>
          ...
          {invoice.billingEmail && <div className="text-xs text-slate-600">{invoice.billingEmail}</div>}
        </div>
        <div>
          <div className="text-[10px] font-semibold uppercase tracking-wider text-slate-500">Billing period</div>
          ...
          <div className="mt-2 text-[10px] font-semibold uppercase tracking-wider text-slate-500">Status</div>
        ...
        <tbody>
          <tr className="border-b">
            <td className="py-3">
              ...
              <div className="text-xs text-slate-500">Equiwings SaaS subscription · {invoice.plan} plan</div>
            </td>
        ...
        <table className="text-sm">
          <tbody>
            ...
            {sameState ? (
              <>
                <tr>
                  <td className="py-1 pr-8 text-right text-slate-600">CGST @ {(halfTaxBps / 100).toFixed(2)}%</td>
                  <td className="py-1 text-right">₹{halfTaxAmount.toLocaleString("en-IN")}</td>
                </tr>
                <tr>
                  <td className="py-1 pr-8 text-right text-slate-600">SGST @ {(halfTaxBps / 100).toFixed(2)}%</td>
                  <td className="py-1 text-right">₹{halfTaxAmount.toLocaleString("en-IN")}</td>
                </tr>
              </>
            ) : (
              <tr>
                <td className="py-1 pr-8 text-right text-slate-600">IGST @ {(invoice.taxBps / 100).toFixed(2)}%</td>
